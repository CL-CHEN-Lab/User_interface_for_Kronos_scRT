                                        ...
                                        shinyFiles::shinyFilesButton(
                                          ...
                                          label = shiny::div(
                                            ...
                                            bsplus::shiny_iconlink() %>%
                                              bsplus::bs_embed_popover(title = 'Select one of the index files' , placement = 'right')
                                          ),
                                          title = 'Bowtie2 index',
                                          style = 'width:100%;',
                                          multiple = F
      ...
      ######metrics
      {
        shinydashboard::tabItem(
          ...
          shiny::fluidRow(shiny::column(
            width = 12,
            shinyjs::hidden(
            shiny::radioButtons(inputId = 'final_table_selector',label = 'Parameter',choices = '',inline = F,width = '100%')
          ))),
          shiny::fluidRow(shiny::column(
            width = 12,
            ...
            shiny::fluidRow(
              shiny::column(
                ...
                shiny::sliderInput(
                  ...
                  label = 'Acceptable mappability range',
                  min = 0.5,
                  max = 3,
                  value = c(0.8, 1.5),
                  step = 0.1,
          ...
          shinydashboard::box(
            ...
            shiny::fluidRow(shiny::column(
              ...
              shiny::numericInput(
                ...
                label = 'Min number of reads per cell',
                value = 2 * 10 ^ 5,
                step = 10 ^ 4,
                width = '100%'
              )
  ...
  ####metrics
  {
    ...
    shiny::observeEvent(input$Load_metrics, {
      if (!is.numeric(input$Load_metrics)) {
        ...
        if (Kronos.scRT::right_format(
          ...
          shiny::updateRadioButtons(session = session,inputId = 'final_table_selector',choices = unique(variables$Metrics$Parameter),inline = T)
    ...
    shiny::observeEvent(variables$info, {
      if (nrow(variables$info) > 0) {
        ...
        shinyjs::enable('Save_plots')
        shinyjs::enable('Save_table')
        shiny::updateRadioButtons(session = session,inputId = 'final_table_selector',choices = unique(variables$info$Parameter),inline = T)
        shinyjs::show('final_table_selector')
      }
    })

    #click plot 1
    # shiny::observeEvent(input$final_plot_click1, {
    #   if(!is.null(input$final_plot_click1)){
    #   Files = variables$Metrics %>%
    #     dplyr::filter(Parameter == input$final_plot_click1$panelvar1) %>%
    #     dplyr::mutate(
    #       d = (Values - input$final_plot_click1$y) ^ 2 + (x - input$final_plot_click1$x) ^
    #         2,
    #       mind = min(d)
    #     ) %>% dplyr::filter(d == mind) %>% dplyr::pull(File)
    #   variables$Plot_selection = variables$Metrics %>%
    ...
    shiny::observeEvent(variables$Metrics, {
      ...
      variables$plot1 = variables$Metrics %>%
        ...
        ggplot2::geom_boxplot(
          ...
          outlier.size =  0,
          outlier.colour = NA,width=0.8
        )+
        ggplot2::geom_point(ggplot2::aes(x, Values,color=Color),alpha=0.5) +
        ggplot2::theme_bw() +
        ggplot2::labs(x = '', y = '')  +
